...
var globalObj={errors:new Array(),
               ...
               cleanUp:function(){
	            ...
	            populateValidationNodes:function(){
	             var i=0;
	             var numberofFrontCounterMachines=Number(dojo.byId("numberofFrontCounterMachines").value);
	       		 var numberofWirelessMachines=Number(dojo.byId("numberofWirelessMachines").value);
	             this.validationNodeList=new Array(new ValidationNode("computerSales","COMPUTER SALES"),
	               						...
	               						for(i=1;i<=numberofFrontCounterMachines;i++){
	              						...
	              						for(i=numberofFrontCounterMachines+1;i<=(numberofFrontCounterMachines+numberofWirelessMachines);i++){
	               						this.validationNodeList[this.validationNodeList.length]=new ValidationNode("BatchRecords_"+(i-1)+"_debit","DEBIT IN WIRELESS MACHINE #"+(i-numberofFrontCounterMachines));
	               						this.validationNodeList[this.validationNodeList.length]=new ValidationNode("BatchRecords_"+(i-1)+"_visa","VISA IN WIRELESS MACHINE #"+(i-numberofFrontCounterMachines));
	               						this.validationNodeList[this.validationNodeList.length]=new ValidationNode("BatchRecords_"+(i-1)+"_mastercard","MASTERCARD IN WIRELESS MACHINE #"+(i-numberofFrontCounterMachines));
	               						this.validationNodeList[this.validationNodeList.length]=new ValidationNode("BatchRecords_"+(i-1)+"_amex","AMEX IN WIRELESS MACHINE #"+(i-numberofFrontCounterMachines));
	               						this.validationNodeList[this.validationNodeList.length]=new ValidationNode("BatchRecords_"+(i-1)+"_batchNumber","BATCH NUMBER IN WIRELESS MACHINE #"+(i-numberofFrontCounterMachines));
	              						}
	              }              
	              }; 
...
//---code for update and format---//
	...
	function updateInStoreAmexTotal(){
       var val=0;
       var num=Number(dojo.byId("numberofFrontCounterMachines").value);
       for(i=0;i<num;i++)
    ...
    function updateBatchRecordTotals(){
      var i=1;
      var num=Number(dojo.byId("numberofFrontCounterMachines").value)+Number(dojo.byId("numberofWirelessMachines").value);
      for(i=1;i<=num;i++)
